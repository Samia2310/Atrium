...
test('participant refunds use their separate notice policy', () => {
  assert.equal(participantRefundPercent(100), 1);
  assert.equal(participantRefundPercent(72), 0.75);
  assert.equal(participantRefundPercent(30), 0.5);
  assert.equal(participantRefundPercent(12), 0);
});
